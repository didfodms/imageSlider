export default class ImageSlider {
  ...
  createIndicator() {
    // docFragment에 li들을 만들어서 ul태그 안에 넣는 작업
    const docFragment = document.createDocumentFragment();
    for (let i = 0; i < this.#slideNumber; i += 1) {
      const li = document.createElement('li');
      // li태그의 data-index 속성값을 i로 설정
      li.dataset.index = i;
  ...
  setIndicator() {
    // 옵셔널 체이닝 ?.
    // index 전부 비활성화 -> 현재 슬라이드인 인덱스 활성화 하는 로직
    // 활성화 된 것은 active 클래스로 표현 (classList를 활용한 클래스 add & remove)
    this.indicatorWrapEl.querySelector('li.active')?.classList.remove('active');
    // nth-child는 ul 태그의 li가 몇번째 child이냐.. 이걸 나타냄
    // nth-child는 1부터 시작하므로 this.#currentPosition + 1을 해주었음
  ...
  onClickIndicator(event) {
    // parseInt 해주는 이유 : event.target.dataset.index가 string타입임
    // 10은 10진수로 바뀌었다라는 뜻
    const indexPosition = parseInt(event.target.dataset.index, 10);
    // indexPosition이 parseInt가 잘 되었다면
